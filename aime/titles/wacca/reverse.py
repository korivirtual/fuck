...
class WaccaReverse(WaccaBase):
    ...
    def handle_user_status_get_request(self, data: Dict) -> List[Any]:
        ...
        # If an omnimix ever comes out for Reverse we'll need to find a way to not send omnimix songs
    ...
    def handle_user_status_getDetail_request(self, data: Dict) -> List[Any]:
        ...
        if "bingo_status" in pdata["profile"]:
            bingo_status = pdata["profile"]["bingo_status"]
        ...
        if "friends" in pdata and pdata["friends"]:
            for friend in pdata["friends"]:
                ...
                if friend_profile is None:
                    self.logger.warn(f"No profile found for friend's game_id {friend} in handle_user_status_getDetail_request")
                    continue
        ...
        for song in songs:
            ...
            if game_song_info or not self.game_config.safe_song_load:
                ...
                if song["version"] <= 3:
                    ...
                    self.util_int_to_bitmask_array(song["fc1"], 5), self.util_int_to_bitmask_array(song["grade"], 13),
        ...
        if user_items:
            for item in user_items:
                try:
                    ...
                    if item[4] == self.ITEM_TYPES["music_difficulty_unlock"] or item[4] == self.ITEM_TYPES["music_unlock"]:
                        ...
                        for x in range(0, 4):
                            if item_data["difficulty"][x] == 1:
                                # FIXME: expert/infirnos don't unlock
                                # id, difficulty, apperance date, unlock date
                                song_unlocks.append([item[5], x + 1, item_data["obtainedDate"] - 1, 
                    ...
                    elif item[4] == self.ITEM_TYPES["touch_effect"]:
                        touch_effect.append([item[5], 1, item_data["obtainedDate"]])

                except:
                    self.logger.error(f"Failed to load item {item['item_id']} for user {user_id}")
        ...
        return [
            [
                ...
                pdata["profile"]["dan_level"], # Dan level
                pdata["profile"]["dan_type"], # Dan type
                pdata["profile"]["wp"], # Reverse Point
                pdata["profile"]["title_part_ids"],
                profile["use_count"], # Total logins
                pdata["profile"]["login_days"], # Number of days logged in
                pdata["profile"]["login_consec"], # Consecutive logins
            ...
            options,
            [[3, 0, pdata["profile"]["play_counts"][0]], [3, 1, pdata["profile"]["play_counts"][1]], [3, 2, pdata["profile"]["play_counts"][2]]],
            [
                ...
                touch_effect # Touch effect info
            ],
            ...
            [pdata["profile"]["xp"],pdata["profile"]["total_wp_gained"],0,cumulative_score,len(titles),len(icons),0,len(note_colors),len(note_sounds),len(plates),0], # TODO: Seasonal info
    ...
    def handle_user_status_create_request(self, data: Dict) -> List[Any]:
        ...
        data={
            "profile": {
                ...
                "last_game_ver": f"{ver_split[0]}.{ver_split[1]}.{ver_split[2]}",
                "play_counts": [0,0,0]
            },
            "option": {
                "note_speed": 5,
        ...
        {"obtainedDate": int(self.srvtime.timestamp()), "uses": 0, "season_uses": 0})        
        self.data.game.put_item(user_id, self.game, self.version, self.ITEM_TYPES["navigator"], 310001, 
        {"obtainedDate": int(self.srvtime.timestamp()), "uses": 0, "season_uses": 0})

        self.data.game.put_item(user_id, self.game, self.version, self.ITEM_TYPES["navigator"], 310002, 
        {"obtainedDate": int(self.srvtime.timestamp()), "uses": 0, "season_uses": 0})
        self.data.game.put_item(user_id, self.game, self.version, self.ITEM_TYPES["navigator"], 210054, 
        {"obtainedDate": int(self.srvtime.timestamp()), "uses": 0, "season_uses": 0})
        self.data.game.put_item(user_id, self.game, self.version, self.ITEM_TYPES["navigator"], 210055, 
        {"obtainedDate": int(self.srvtime.timestamp()), "uses": 0, "season_uses": 0})
        self.data.game.put_item(user_id, self.game, self.version, self.ITEM_TYPES["navigator"], 210056, 
        {"obtainedDate": int(self.srvtime.timestamp()), "uses": 0, "season_uses": 0})
        self.data.game.put_item(user_id, self.game, self.version, self.ITEM_TYPES["navigator"], 210057, 
        {"obtainedDate": int(self.srvtime.timestamp()), "uses": 0, "season_uses": 0})
        self.data.game.put_item(user_id, self.game, self.version, self.ITEM_TYPES["navigator"], 210058, 
        {"obtainedDate": int(self.srvtime.timestamp()), "uses": 0, "season_uses": 0})
        self.data.game.put_item(user_id, self.game, self.version, self.ITEM_TYPES["navigator"], 210059, 
        {"obtainedDate": int(self.srvtime.timestamp()), "uses": 0, "season_uses": 0})
        self.data.game.put_item(user_id, self.game, self.version, self.ITEM_TYPES["navigator"], 210060, 
        {"obtainedDate": int(self.srvtime.timestamp()), "uses": 0, "season_uses": 0})
        self.data.game.put_item(user_id, self.game, self.version, self.ITEM_TYPES["navigator"], 210061, 
        {"obtainedDate": int(self.srvtime.timestamp()), "uses": 0, "season_uses": 0})
    ...
    def handle_user_trial_get_request(self, data: Dict) -> List[Any]:
        ...
        dan_list =  [
                ...
                [3012,12,0,0,[0,0,0], 1],
                [3013,13,0,0,[0,0,0], 1],
                [310001,0,0,0,[0,0,0], 1],
                [310002,0,0,0,[0,0,0], 1],
                [310003,0,0,0,[0,0,0], 1],
                [310004,0,0,0,[0,0,0], 1],
                [310005,0,0,0,[0,0,0], 1],
                [310006,0,0,0,[0,0,0], 1],
            ]
    ...
    def handle_user_music_update_request(self, data: Dict) -> List[Any]:
        ...
        if profile is None:
            self.logger.warn(f"No profile for game_id {game_id} in handle_user_music_update_request", extra={"game": "WaccaReverse"})
            return []
        ...
        score = song_info[3]
        difficulty = song_info[1]
        marvelous = song_info[4][0]
        great = song_info[4][1]
        good = song_info[4][2]
        miss = song_info[4][3]
        ...
        if not old_score:
            ...
            return [
                ...
                [[0,0], [1,0], [2,0], [3,0], [4,0]] # ranking info [type, rank]
            ]
            ...
            best_grade = self.util_int_to_bitmask_array(old_score["grade"], 13)
    ...
    def handle_user_mission_update_request(self, data: Dict) -> List[Any]:
        game_id = data["params"][0]
        sheet_num = data["params"][1][0]
        mission_progress = data["params"][1][1]
        get_items: List = data["params"][2]
        tutorial_status = data["params"][3]
        ...
        if len(get_items) > 0:
            self.util_put_items(game_id, pdata, get_items)
        
        pdata["profile"]["bingo_status"] = [sheet_num, mission_progress]
